...
def validate_array_file(filepath: str) -> bool:
    """
    Validate that an NPY file is readable and contains valid data.
    
    Why: Ensures array files are not corrupted before use in training.
    This prevents runtime errors during dataset loading.
    
    What: Attempts to load the NPY file and checks for:
    - File exists and is readable
    - Array is not empty
    - Array contains only finite values (no NaN/Inf)
    - Array has valid shape (2D for spectrograms/masks)
    
    Args:
        filepath: Path to NPY file to validate.
    
    Returns:
        bool: True if file is valid, False otherwise.
    """
    if not os.path.exists(filepath):
        logger.warning(f"Array file does not exist: {filepath}")
        return False
    ...
    try:
        ...
        if array.size == 0:
            logger.warning(f"Array file is empty: {filepath}")
            return False
        
        if not np.all(np.isfinite(array)):
            logger.warning(f"Array contains NaN or Inf: {filepath}")
            return False
        
        if array.ndim != 2:
            logger.warning(
                f"Array has unexpected shape: {array.shape}, expected 2D: {filepath}"
            )
            return False
        ...
        logger.error(f"Error validating array file {filepath}: {str(e)}")
        return False


def list_samples_in_scenario(
    base_dir: str,
    freq_range: Tuple[float, float],
    scenario_name: str
) -> List[Dict[str, str]]:
    """
    List all samples in a specific scenario and frequency range.
    
    Why: Provides a convenient way to get samples for a specific scenario
    without scanning the entire dataset.
    
    What: Calls scan_directory_for_samples() with specific filters and returns
    the results. This is a convenience wrapper for common use case.
    ...
    Returns:
        List[Dict[str, str]]: List of sample dictionaries for the specified scenario.
    
    Raises:
        OSError: If base directory cannot be accessed.
    """
    return scan_directory_for_samples(base_dir, freq_range, scenario_name)
